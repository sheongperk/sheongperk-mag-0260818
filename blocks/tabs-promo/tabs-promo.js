...
export default async function decorate(block) {
  ...
  tablist.id = `tabs-promo-list-${tabBlockCnt += 1}`;
  ...
  tabHeadings.forEach((tab, i) => {
    const id = `tabs-promo-panel-${tabBlockCnt}-tab-${i + 1}`;

    // decorate tabpanel
    const tabpanel = block.children[i];
    tabpanel.className = 'tabs-promo-panel';
    tabpanel.id = id;
    ...
    button.className = 'tabs-promo-tab';
    button.id = `tab-${id}`;
